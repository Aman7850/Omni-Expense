...
function FormScreen({route, navigation}) {
  ...
  const insertData = () => {
    console.log(
      ...
      ExpenseReference,
      uid,
      'this is going new in database!',
    );
    {
      setvisible(true);
      db.transaction(function (tx) {
        tx.executeSql(
          'INSERT INTO Omni_Expense(Description,Products,unitPrice,Quantity,Total,PaidBy, Photo_id, CreatedDate, Status,CurrencySymbol,ExpenseReference, uId, submittedBy) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
          [
            description,
            products,
            unitprice,
      ...
      db.transaction(tx => {
        ...
        tx.executeSql(
          ...
          (tx, results) => {
            ...
            tx.executeSql(
              `SELECT * FROM Omni_Expense_Image`,
              [],
              (tx, results) => {
                console.log(results.rows.length, 'FOCUS222!');
                setInputValue(false);
                setvisible(false);
                ...
                console.log(
                  temp1,
                  'CUSTOMER ID HERE! AND I AM OUT! OMNI EXPENSE CHANCK AGIAN',
                );
              ...
              <View
                ...
                <View style={{alignContent: 'center'}}>
                  <Title style={{alignSelf: 'center'}}>
                    {CurrencySym} {(unitprice * Quantity).toFixed(2)}
                  </Title>
                </View>
              </View>
            </>
